package edu.hust.xzf.mutator.fixpatterns;
        ...
        for (ITree suspVar : suspVars) {
            ...
            if ("boolean".equals(suspVarType)) {
                ...
                if (sameTypeVars != null) {
                    ...
                    MyFor:
                    for (Pair<String, Double> var : similarVarPairs) {
                        for (ITree suspV : suspVars) {
                            if (suspV.getLabel().equals(var.getFirst())) continue MyFor;
                        }
                        if (!isQualifiedName && Character.isUpperCase(var.getFirst().charAt(0)))
                            continue MyFor; //var.getFirst().contains(".") ||
...
//				}
    ...
    private boolean isForStatementVar(String suspVarName) {
        ...
        while (true) {
            ...
            if (Checker.isForStatement(parentStmt.getType()) || Checker.isEnhancedForStatement(parentStmt.getType()))
                break;
...
//	}
    ...
    private final Map<String, ITree> sameValueSuspVarTrees = new HashMap<>();

    private static class LongestCommonSubsequence {
